...
class dl_model():
	...
	def predict(self, string, misses, char_to_id):
		...
		:param misses: list of characters which were predicted but game feedback indicated 
		...
		#pass through model
		output = self.model(inputs, input_lens, \
			miss_encoded).detach().cpu().numpy()[0]

		#sort predictions
		sorted_predictions = np.argsort(output)[::-1]
		
		# we cannnot consider only the argmax since a missed character 
		# may also get assigned a high probability
