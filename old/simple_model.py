...
class SimpleLSTM(BaseModel):
    ...
    def calculate_loss(self, model_out, labels,
        ...
        # # Mask loss for actual sequence length
        # seq_lens_mask = torch.arange(model_out.size(1), device=self.device).expand(
    ...
    # def calculate_loss(self, model_out, labels, input_lens, miss_chars, vocab_size):
    ...
    #     one_hot_labels = torch.zeros(batch_size, seq_len,
    #                                  vocab_size, device=self.device)

    #     print(one_hot_labels.shape)
    #     print(labels.unsqueeze(-1).shape)
    #     one_hot_labels.scatter_(2, labels.unsqueeze(-1), 1)

    #     # Apply log softmax to model output
    #     outputs = F.log_softmax(model_out, dim=-1)

    #     # Ensure miss_chars has the correct shape before expanding
    #     if miss_chars.dim() == 3:
    #         miss_chars = miss_chars.squeeze(-1)

    #     # Now expand miss_chars to match outputs dimensions
    #     miss_chars = miss_chars.unsqueeze(-1).expand_as(outputs)
