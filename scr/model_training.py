...
def validate_hangman(model, data_loader, char_frequency, max_word_length,
    ...
    game_counts_per_length = defaultdict(int)
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            for idx, full_word in enumerate(batch_full_words):
                ...
                char_word_statistics[full_word]["correctly_guessed"] = char_word_statistics[full_word]["correct_attempts"] == len(
                    full_word)

            # Game simulation for unique words with adaptive sampling
            for full_word in batch_full_words:
                word_length = len(full_word)
                if game_counts_per_length[word_length] < max_games_per_length and full_word not in used_words:
                    ...
                    game_total_count += 1
                    game_counts_per_length[word_length] += 1
...
#                     won, final_word, attempts = play_game_with_a_word(
#                         model, full_word, char_frequency, max_word_length,
#                         device, max_attempts, normalize)
#                     game_word_statistics[full_word] = {
#                         "won": won, "final_word": final_word, "attempts": attempts}
#                     word_length = len(full_word)
#                     game_length_statistics[word_length]["games"] += 1
