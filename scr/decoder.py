...
class SimpleLSTM(nn.Module):
    ...
    def forward(self, fets, original_seq_lens, missed_chars):
        ...
        missed_chars_processed = missed_chars_processed[:, :unpacked_output.size(
            1), :]
        # print(
        #     f"Missed character process shape after processing: {missed_chars_processed.shape}")
        ...
        combined = torch.cat([unpacked_output, hidden.unsqueeze(1).repeat(
            1, unpacked_output.size(1), 1), missed_chars_processed], dim=2)
...
def test_simple_lstm():
    ...
    batch_size = 10
    max_seq_length = 10
    max_word_length = 29  # From your fixed parameters
    embedding_dim = 50  # From your fixed parameters
    additional_state_features = 154 - \
        (max_word_length * 5)  # Calculated as per your code
    input_dim = max_word_length * embedding_dim + \
        additional_state_features  # From your code
    hidden_dim = 256  # Assuming a hidden_dim for the LSTM
    output_dim = 28  # From your fixed parameters
    num_layers = 2  # Assuming a number of layers for the LSTM
    missed_char_dim = 28  # From your fixed parameters
    ...
    original_seq_lens = torch.randint(
        1, max_seq_length + 1, (batch_size,)).sort(descending=True).values
